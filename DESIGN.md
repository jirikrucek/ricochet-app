---
version: alpha
name: Ricochet App Design System
description: A warm, generous app anchored on a clean white canvas and the brand primary (#ff385c), the single color voltage that carries every primary CTA, search-button orb, and rating dot. Type runs Inter at modest weights — display sits at 22–28px in weight 500/600 rather than the heavy 700+ that fintech and enterprise systems use; the brand trusts generous whitespace over typographic muscle. Two product sections (Players, Tournaments) sit in the top nav. Pill-shaped search bars (`{rounded.full}`), softly rounded cards (`{rounded.lg}` ~14px), and 32px button radii read as friendly and human — there is no hard corner anywhere except the body grid.
...
typography:
  display-xl:
    fontFamily: "'Inter', system-ui, -apple-system, Roboto, 'Helvetica Neue', sans-serif"
    fontSize: 28px
    fontWeight: 700
    lineHeight: 1.43
    letterSpacing: 0
...
The Ricochet App design system is a clean, human-scaled interface built on a white canvas. The base is **pure white** (`{colors.canvas}` — #ffffff) with deep near-black ink (`{colors.ink}` — #222222) for headlines and body, and a single voltage of **Crimson** (`{colors.primary}` — #ff385c) carrying every primary CTA, the search-button orb, the heart save state, and inline brand links. There is no secondary brand color in mainline views — the **Luxe purple** (`{colors.luxe}` — #460479) and **Plus magenta** (`{colors.plus}` — #92174d) tokens are sub-brand accents that only appear inside the Luxe / Plus sub-brand contexts.

Type runs **Inter**, a variable font loaded from Google Fonts, with a system stack underneath. Inter sits at modest weights — display headlines render at 22–28px in weight 500–600, not the heavy 700+ weights that financial or enterprise systems lean on. The hero h1 at 28px / 700 works because generous whitespace and content cards carry visual weight rather than typographic muscle.
...
- Single accent color: `{colors.primary}` (#ff385c — "Crimson") carries every primary CTA, the search orb, the heart save state, and the brand wordmark. Used scarcely — most pages are 90% white + ink with one or two Crimson moments.
- Custom variable type: `Inter`. Display weights sit at 500–700, body at 400. Modest weight is intentional — the system trusts content for visual heft.
- Two-section top nav: Players, Tournaments — each link uses an underline rule on the active state (`{component.product-tab-active}`).
- Pill-shaped global search bar: white surface, fully rounded (`{rounded.full}`), divided by 1px hairlines into segments, terminated by a circular primary search orb (`{component.search-orb}`).
...
- **Crimson** (`{colors.primary}` — #ff385c): The single brand color. Used for primary CTA backgrounds (Reserve, Continue), the search orb, the heart save state on cards, and inline brand links.
- **Crimson Active** (`{colors.primary-active}` — #e00b41): The press / pointer-down variant — slightly more saturated. Used on `{component.button-primary-active}`.
- **Crimson Disabled** (`{colors.primary-disabled}` — #ffd1da): A pale tint used on disabled CTAs.
- **Luxe Purple** (`{colors.luxe}` — #460479): Sub-brand accent for the Luxe tier. Only appears inside Luxe-branded surfaces — never in mainline views.
- **Plus Magenta** (`{colors.plus}` — #92174d): Sub-brand accent for the Plus tier. Same scoping as Luxe — sub-product only.

### Surface

- **Canvas** (`{colors.canvas}` — #ffffff): The default page floor for every public page. The app does not currently have a dark mode.
...
- **Star Rating** (`{colors.star-rating}` — #222222): The same ink token — the star icon and rating numbers all render in ink rather than a yellow/gold color, which is a deliberate brand choice (yellow stars feel cheap in a sport context).
- **On Primary** (`{colors.on-primary}` — #ffffff): White text on Crimson CTAs.

### Semantic

- **Error** (`{colors.primary-error-text}` — #c13515): Inline error text for form validation. Distinct from Crimson — slightly darker, more saturated red.
...
The system runs **Inter** for everything — display, body, navigation, captions, microcopy. Fallbacks walk `-apple-system, system-ui, Roboto, "Helvetica Neue", sans-serif`.
...
| `{typography.title-sm}`       | 16px | 500    | 1.25        | 0                  | Footer column heads ("Support", "Hosting", "About")            |
...
| `{typography.caption-sm}`     | 13px | 400    | 1.23        | 0                  | Footer legal line ("© 2026 Ricochet App")                      |
...
Inter is used at its default metrics across the full type scale — no line-height or letter-spacing adjustments are needed beyond the values declared per token.
...
- **Footer:** 3-column link list (Support / Hosting / About) at desktop, collapsing to 1-column on mobile.
...
**`button-primary`** — Crimson fill, white text, 8px radius, 14×24px padding, 48px height, weight 500. The most common CTA across the system: "Reserve", "Continue", "Search", account-flow primaries.

**`button-primary-active`** — The press state. Background flips to `{colors.primary-active}`. No transform, no shadow change.

**`button-primary-disabled`** — Pale Crimson tint at #ffd1da with white text. Cursor not-allowed.

**`button-secondary`** — White fill with ink text and a 1px ink outline. 8px radius. Used for "Save", "Cancel", and inverse CTAs over Crimson surfaces.
...
**`button-pill-primary`** — A pill-shaped Crimson CTA used on featured cells — 9999px radius, 10×20px padding, 14px label.
...
**`search-orb`** — The circular primary orb terminating the right edge of the search bar. 48×48px, fully rounded, white magnifying-glass icon centered. The single strongest color moment on the page.

### Top Navigation

**`top-nav`** — White surface, 80px height, 1px bottom hairline. The app wordmark sits flush left, the primary nav links (Players / Tournaments) sit in the center, and account utilities sit flush right.
...
**`property-card`** — A photo-first card. 1:1 aspect-ratio image with `{rounded.md}` corner clipping, image carousel dots overlay, "Guest favorite" floating badge top-left (`{component.guest-favorite-badge}`), and a heart icon top-right (`{component.icon-button-circle}` in default outlined state, Crimson-filled when saved). Beneath the image: 4–5 lines of meta — title (`{typography.title-md}`), distance / dates (`{typography.body-sm}` muted), and price ("$X night") right-aligned.
...
**`footer-light`** — White surface (matches the page canvas — the app has no contrast footer), 48×80px padding. Three columns of link blocks (Support / Hosting / About), separated by generous 24px gutters. Each column heads with a `{typography.title-sm}` ink label and stacks `{component.footer-link}` rows in `{typography.body-sm}` ink.
...
- **Hover state colors:** intentionally not documented per the global no-hover policy — the design system's `:hover` styling for cards is a subtle elevation lift, but precise values are not yet specified.
- **Loading states / skeleton screens:** not visible on the extracted surfaces.
- **Map view styling:** the search-results map uses Mapbox-tinted tiles with custom primary-color markers; not captured here.
